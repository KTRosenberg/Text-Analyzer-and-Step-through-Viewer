...
def binary_min_line_search(line_numbers, low, high, starting_line):
    print(line_numbers)
    mid = 0
...
params: string all_text
        ...
        list word_count (contains information about the chosen word):
            ...
            [2], contains list of list[ints] with miscellaneous information:
                ...
                [2][1] contains list[ints] of number of words (in entire text) that occur before the given instance (i) of the word
...
"""
def word_step(all_text, new_line_positions, word_count, starting_line):
    ...
    if starting_line == 1:
        ...
        while index < num_lines:
            if line_numbers[index] >= starting_line:
                found = index
                break
            index += 1
        """
    ...
    while w_inst_index < num_word_inst:
        ...
        print('\n' + ' '*(word_marker - line_start) + '^')
        print("number of words between instances of word: " + str(num_words_between[w_inst_index]))
        choice = input("\nL" + str(current_line) + ">> ")
            ...
            if cur_step < step:
                ...
                while True:
                    try:
                        ...
                        cur_step = 0
                        if step == __W_NEXT_INST:
                            step = word_step(all_text, new_line_positions, word_count, current_line)
                        elif step == __ENTER:
                            step = 1
                        else:
                            step = int(step)
...
return: list analysis_list of word_freq dictionary and optional dictionaries
    word counts and miscellaneous word/text information:
...
def main():
    ...
    while prompt:
        print(word_counts)
        word = input("Please select a word (enter 0 to quit): ")
        if word == '0':
            prompt = False
