...
"""
def calc_w_frequency(all_text, clean=0, max_len=0, trivial=1, trivial_list=[], gender=0, gender_terms=[], mood=0, mood_terms=[], in_word_punct=["'", '-'], eq_words={"can't":["can", "not"], "cannot":["can", "not"], "won't":["will", "not"], "shouldn't":["should", "not"]}):
...
def main():
    ...
    while loop:
        ...
        if option == '' or option == '1':
            ...
            _encoding = input("Enter the encoding of the file, (enter or 1/0 or any other entry), ascii by default, 2 for utf-8: ") 
