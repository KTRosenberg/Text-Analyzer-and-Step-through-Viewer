...
def word_step(all_text, new_line_positions, word_count, starting_line=1):
    ...
    choice = ''
    word_marker = 0
    while w_instance < num_word_inst:
        if all_text[w_pos] != '\n':
            if w_pos == pos_word_inst[w_instance]:
                word_marker = w_pos
        ...
        choice = input("l" + str(current_line) + ", number of words between instances of word: " + str(num_words_between[w_instance]) + " >> ")

        if choice == '>' or choice == '':
            w_instance += 1
            if w_instance == num_word_inst:
                break
            w_pos = line_start = new_line_positions[line_numbers[w_instance]-2] + 1
            current_line = line_numbers[w_instance]
        elif choice == '<':
            if w_instance > 0:
                w_instance -= 1
                if line_numbers[w_instance] > 1:
                    w_pos = line_start = new_line_positions[line_numbers[w_instance]-2] + 1
                    current_line = line_numbers[w_instance]
                else:
                    w_pos = line_start = 0
            ...
            if double_punct == 2 or is_valid_char(char, in_word_punct) == False:
                ...
                if len(new_word) > 0:
                    ...
                    if joined_word not in word_freq:
                        ...
                        #distance from start to previous instance of word
