...
"""
TO-FIX: oddities in word_step when chosen word is first word in text,
        need error checking for input of alphabetical characters where there should be none
"""
import sys
import os
...
def word_step(all_text, new_line_positions, word_count, starting_line, choice='>'):
    ...
    if starting_line > 1 and choice == __W_NEXT_INST:
        ...
        if found == -1:
            print("HUUUUUUH2")
            print("no instance before, starting at first instance\n")
        
    if found >= 0:
        if line_numbers[found] > 1:
            w_inst_index = found
    ...
    while w_inst_index < num_word_inst:
        ...
        if choice == __W_NEXT_INST:
            print("w skipped forwards: " + str(num_words_between[w_inst_index]))
        elif choice == __W_PREV_INST:
            print("w skipped backwards: " + str(num_words_between[w_inst_index + 1]))
