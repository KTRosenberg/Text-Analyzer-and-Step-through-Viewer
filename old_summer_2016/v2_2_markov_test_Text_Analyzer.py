...
def simple_markov_test(word_list, prefix_len=2):
    markov_dict = {}
    prefix = ()
    word   = ()
    i = 0
    while i < len(word_list):
        ...
        if len(prefix) < prefix_len:
            ...
            try:
                markov_dict[prefix].append(word)
            except KeyError:
                markov_dict[prefix] = [word]
...
def random_markov(markov_dict, n):
    start = random.choice(list(markov_dict.keys()))
    j = 0
    for i in range(n):
        suffixes = markov_dict.get(start, None)
        if suffixes == None:
            random_markov(n-i)
        else:
...
def main():
    ...
    if choices_list[0] == 1:
        ...
        markov_dict = simple_markov_test(analysis_dict["word list"], 2)
        print()
        random_markov(markov_dict, 100)
