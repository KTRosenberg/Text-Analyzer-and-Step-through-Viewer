        ...
        for char in current_text:
            ...
            if double_punct == 2 or is_valid_char(char, in_word_punct) == False:
                ...
                if len(new_word) > 0:
                    ...
                    #if the new word has not been added to the dictionary and the word is alphabetical,
                    #add an entry for the word in the word list and in the dictionary with a count of 1
                    if joined_word not in word_counts:
                        word_counts[joined_word] = [1, [word_i]]
                        #add new word to word list
                        word_list_append(joined_word)
                    ...
                    #increment the frequency count and other information for that word
                        ...
                        #increment the word frequency count
                        word_counts[joined_word][0] += 1
                        word_counts[joined_word][1].append(word_i)
                #reset the word string
                del new_word[:]

    word_analysis["word list"] = word_list

    print("WORD LIST: ", word_list)

    print("WORD COUNTS: ", word_counts)
    word_analysis["word counts"] = word_counts
...
def main():
    ...
    while choose_file:
        ...
        while choose_file:
            ...
            option = input("Would you like to add a file? (enter or 1/0 or any other entry): ")
        ...
        print("CHOSEN FILES: ", text_list)
    """

    file_options = next(os.walk(os.getcwd()))[2]

    for i in range (1, 14):
        text_file = open(file_options[i], 'r', encoding='utf-8')
        next_text = text_file.read()
        text_file.close()
        text_list.append(next_text)
        
    analysis_dict = word_counts(text_list)
    ...
    word_count = analysis_dict["word counts"]
    count = 0
    line_number = 0
    format_start = '{:<' + str(len_longest) + '} {:>'
    #format words and counts nicely
    for word in sorted(word_count.keys()):
        word_data = word_count[word]
        count = word_data[0]
        inst_i = word_data[1]
        print(str( format_start + str(len(str(count))) + '}').format(word, count), end='')
        print()
        #print(", word instances: ", inst_i)
