...
params: string all_text
        ...
        [freq, [line_nums], [[w_between_inst],[w_before_inst],[pos_word_inst]] ]
"""
def text_step(text_as_lines, word_analysis_given_word):
...
return: list analysis_dict of word_analysis dictionary and optional dictionaries
    word counts and miscellaneous word/text information for each word:
        access word count dictionary with with analysis_dict[0]
        select a word with analysis_dict[0][chosen_word]
            [0], contains word frequency count
            [1], contains list [ints] of line numbers for each instance of the word
            [2], contains list of list[ints] with miscellaneous information:
                [2][0] contains list[ints] of number of words between instance (i) and instance (i + 1) of the word
    ...
    gender:
        access counts with (analysis_dict[2])[m] and (analysis_dict[2])[f]
        access percentages with (analysis_dict[2])[%_m] and (analysis_dict[2])[%_f]
        access percent of words identifiable as masculine or feminine with (analysis_dict[2])[%_indentifiable]
    mood:
        access counts with (analysis_dict[3])[:D] and (analysis_dict[3])[D:]
        access percentages with  (analysis_dict[3])[%_:D] and (analysis_dict[3])[%_D:]
        access percent of words identifiable as happy or sad with (analysis_dict[3])[%_indentifiable]
"""
...
def main():
    ...
    all_text = ''
    try:
        analysis_dict = calc_w_frequency(text_file, choices_list[0], choices_list[1], choices_list[2], [], choices_list[3], [], choices_list[4], [])  
    except:
        sys.exit("ERROR")
    ...
    if choices_list[0] == 1:
        ...
        while prompt:
            ...
            if word == '0':
                ...
                text_step(all_text, analysis_dict[4], word_counts[word])
            else:
